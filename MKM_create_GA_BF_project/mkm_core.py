...
class LithotypeInterval:
    """Непрерывный интервал по глубине с одним литотипом и его матрицей свойств."""
    interval_id: int
    lithotype: int
    start_idx: int
    end_idx: int
    ...
    @property
    def size(self) -> int:
        """Количество строк (отсчётов глубины) внутри интервала."""
        return int(len(self.row_indices))


def resolve_path(path_value: str | Path, base_dir: Path | None = None) -> Path:
    """Преобразует относительный путь в абсолютный относительно `base_dir`/`PROJECT_ROOT`."""
...
def infer_property_mnemonics(
    ...
    """Проверяет и возвращает 4 мнемоники свойств для построения 5x5 МКМ."""
    keyset = set(las_keys)
...
def load_mkm_from_las(
    ...
    *,
    ...
    """Читает LAS и готовит массив `[depth, litho, 4 свойства, 1]` + маски и срезы по литотипам."""
...
def split_lithotype_intervals(data: np.ndarray) -> list[LithotypeInterval]:
    """Делит входной массив на интервалы, где код литотипа не меняется."""
...
def calc_mkm_model_by_intervals(
    ...
    """Собирает итоговую МКМ-модель: на каждый интервал применяется своя матрица."""
...
def calc_metrics_mkm(mkm_model: np.ndarray) -> tuple[float, float, float]:
    """Считает три целевые метрики качества для МКМ-модели."""
...
def calc_quality_score(
    ...
    """Собирает общий score как взвешенную сумму трёх метрик."""
    return (
...
def calc_coll_metrics(matrix: np.ndarray, coll_prop: np.ndarray) -> tuple[float, float]:
    """Локальные метрики качества для коллекторных строк."""
...
def calc_glin_metrics(matrix: np.ndarray, glin_prop: np.ndarray) -> tuple[float, float]:
    """Локальные метрики качества для глинистых строк."""
    inv_m = np.linalg.inv(prepare_mkm_matrix_for_application(matrix))
...
def plot_with_sign(
    ...
    """Рисует один трек: положительные значения линией, отрицательные — маркерами."""
    pos_mask = x >= 0
    neg_mask = ~pos_mask
...
def save_mkm_plot(
    ...
    *,
    ...
    """Сохраняет PNG-график МКМ по глубине с треками компонент и границами интервалов."""
    depth = mkm_model[:, 0]
...
def validate_matrix_shape(matrix: np.ndarray, matrix_name: str) -> None:
    """Проверяет, что матрица имеет форму 5x5."""
    if matrix.shape != (5, 5):
...
def flatten_bounds(a_min: np.ndarray, a_max: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Преобразует две матрицы границ 5x5 в плоские векторы длины 25."""
...
def validate_k_shape(k_matrix: np.ndarray, matrix_name: str) -> None:
    """Проверяет корректность матрицы дискретизации `a_k_*`."""
