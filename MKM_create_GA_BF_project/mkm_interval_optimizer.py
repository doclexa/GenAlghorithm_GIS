...
"""Интервальная оптимизация МКМ: подбор матриц отдельно для каждого литотип-интервала."""
...
class IntervalOptimizationResult:
    """Результат оптимизации одного интервала (BF или GA)."""
    interval_id: int
    lithotype: int
    start_idx: int
    end_idx: int
...
class IntervalGenerationQualityPoint:
    """Точка кривой качества: состояние общей модели на шаге поколения GA."""
    generation: int
    quality_score: float
    negative_share: float
...
class IntervalOptimizationSummary:
    """Итог по скважине: метрики, матрицы интервалов и служебная статистика метода."""
    method: str
...
def _interval_bad_metric_name(lithotype: int) -> str:
    """Возвращает имя «плохой» метрики для текущего литотипа."""
...
def _run_single_interval_bruteforce(
    ...
    *,
    ...
    """Оптимизирует один интервал полным перебором и возвращает подробный результат."""
...
def run_interval_bruteforce(
    *,
    ...
    """Запускает BF по всем интервалам одной скважины и собирает общую сводку."""
    interval_results: list[IntervalOptimizationResult] = []
    interval_matrices: dict[int, np.ndarray] = {}
...
def _run_single_interval_ga(
    ...
    *,
    ...
    """Оптимизирует один интервал генетическим алгоритмом."""
    started = time.perf_counter()
...
def _state_for_generation(
    ...
    """Берёт лучшее известное состояние интервала на заданном поколении."""
...
def build_interval_ga_quality_curve(
    *,
    ...
    """Строит кривую качества общей модели по поколениям интервального GA."""
...
def run_interval_ga(
    *,
    ...
    """Запускает интервальный GA по всем интервалам и считает итоговые метрики скважины."""
    interval_results: list[IntervalOptimizationResult] = []
    interval_matrices: dict[int, np.ndarray] = {}
...
def write_interval_results_csv(results: list[IntervalOptimizationResult], csv_path: Path) -> None:
    """Сохраняет детальные результаты по интервалам в CSV."""
...
def save_interval_matrices_npz(summary: IntervalOptimizationSummary, npz_path: Path) -> None:
    """Сохраняет матрицы интервалов в NPZ (`interval_<id>`)."""
