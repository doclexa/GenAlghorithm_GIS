...
def clip_individual(individual: list[float], lower: np.ndarray, upper: np.ndarray) -> None:
    """Ограничивает гены особи диапазонами нижних/верхних границ."""
...
def ensure_deap_classes() -> None:
    """Лениво регистрирует DEAP-классы fitness/individual (без повторного создания)."""
...
def evaluate_coll_individual(
    ...
    """Фитнес-функция для особи коллектора: штрафуем отрицательность и плохую долю коллектора."""
...
def evaluate_glin_individual(
    ...
    """Фитнес-функция для особи глин: штрафуем отрицательность и плохую долю глин."""
    matrix = np.array(individual, dtype=float).reshape(5, 5)
    try:
...
@dataclass
class GAParams:
    """Параметры запуска GA для одной группы (COLL/GLIN)."""
    population_size: int
    ngen: int
    cxpb: float
    mutpb: float
...
class GroupGenerationState:
    """Состояние лучшего решения группы на конкретном поколении."""
    generation: int
...
@dataclass
class GroupGAResult:
    """Итог оптимизации группы (COLL/GLIN) с историей по поколениям."""
    best_matrix: np.ndarray
    best_score: float
    neg_share: float
    bad_share: float
...
def _empty_group_result() -> GroupGAResult:
    """Пустой результат для случая, когда группа не содержит строк."""
...
def build_toolbox(
    ...
    """Собирает DEAP toolbox с bounded crossover/mutation."""
    ensure_deap_classes()
...
def run_ea_with_patience(
    ...
    """Запускает эволюцию с early stopping по `patience/min_delta`."""
...
def run_group_ga(
    ...
    """Запускает GA для одной группы (COLL или GLIN) и собирает историю поколений."""
...
def optimize_mkm_with_ga(
    ...
    """Оптимизирует матрицы для COLL и GLIN в одном вызове и возвращает время поиска."""
